...
| **Real BMAD upstream (PR #69)** | weekly CI | `.github/workflows/bmad-compat.yml` extended — runs `/bmad-next --doctor` AND `/bmad-loop --plan-first` AND the skill-frontmatter sanity sweep against the actual BMAD upstream every Monday. |
| **Layer 1 Claude sim (PR #69)** | bi-weekly opt-in CI | `.github/workflows/layer1-sim.yml` + `tests/harness/layer1-claude-sim.test.ts` — sends each SKILL.md to the real Claude API and asserts Claude's intended bash invocation matches the prescribed Layer-2 entry point. Skipped without `ANTHROPIC_API_KEY` (set as repo secret to enable). Costs ~$0.05 per run on Haiku. |

CI: `.github/workflows/ci.yml` runs `bun run check` (= `biome ci . && bun test --pass-with-no-tests`) on `ubuntu-latest`. The weekly `bmad-compat.yml` job runs `/bmad-next --doctor` + `/bmad-loop --plan-first` against the latest BMAD upstream and files an issue on regression. The bi-weekly `layer1-sim.yml` job sends SKILL.md to the Claude API (opt-in via `ANTHROPIC_API_KEY` secret). ~16 seconds for the full PR suite locally; comparable in CI.
...
| `bmad-compat.yml` doctor + plan-first + frontmatter sweep | Weekly + `workflow_dispatch` | CI cron | **active** — files issue on failure (PR #69 extended) |
| `layer1-sim.yml` Claude API harness | Bi-weekly + `workflow_dispatch` | CI cron | **active opt-in** — set `ANTHROPIC_API_KEY` repo secret to enable; otherwise skips cleanly (PR #69 added) |
